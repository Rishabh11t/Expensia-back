...
export const getBudgetUsageThisMonth = async (req, res) => {
  ...
  const userId = new Budget.db.base.Types.ObjectId(req.user._id);
    let expenseAgg = [];
    try {
      expenseAgg = await Transaction.aggregate([
        {
          $match: {
            user: userId,
            type: 'Expense',
      ...
      return res.status(500).json({ message: 'Aggregation failed', error: aggErr.message });
    }
